...
class FileManager {
    ...
    createUploadStorageSession = () => {
        const prefix = id_prefixer[Math.floor(Math.random() * id_prefixer.length)];
            ...
            fs.mkdir(uploadPath + id, () => {
                ...
                if ( request.payload['file'].length) {
                    for (var i = 0; i < request.payload["file"].length; i++) {
                        ...
                        request.payload["file"][i].pipe(fs.createWriteStream(uploaddir + "/" + request.payload["file"][i].hapi.filename))
                        // console.log(request.payload["file"][i]);
                    }
                } else {

                    request.payload["file"].pipe(fs.createWriteStream(uploaddir + "/" + request.payload["file"].hapi.filename))
                    // console.log(request.payload["file"][i]);
